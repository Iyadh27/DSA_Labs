...
struct HashTable{
	...
	//add your code below
        ...
        //check if the slot is empty and proceed
        if(empty){
            cout << "User not found\n";
        }
        else{
            //find the user in the linked list
            Node* node = password[hash].head;
            while(node != NULL){
                if(node->user_name == user_name){
                    //user found in linked list
                    cout << "User found: " << node->password << "\n";
                    break;
                }
                //if not found jump to next link
                node = node->link;
            }
            //if the node is empty display that the user not found
            if(node == NULL){
                cout << "User not found\n";
            }
        }
