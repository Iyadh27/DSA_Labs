...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        int hash = 0;
        //add your code below
        //iterate over the elements in the user_name and add up their ascii values
        for (int i = 0; i < user_name.length(); i++){           
            sum += user_name[i];
        }
        //get the mod value of the sum for the MAX_LENGTH and calculate the hash value
        hash = sum % MAX_LENGTH;
    ...
    //add an if condition to complete the code here
        //check whether the slot is empt and add the value if it isnt
        if(empty){
    ...
    void hash_lookup(string user_name){
        ...
        empty = is_slot_empty(hash);
    //add an if condition to complete the code here
        //chack whether the item is actually in the hash table and then print it if it is available

        if(empty){
            cout<<"No item found\n";
        }
        else{
            cout<<"User found: "<<password[hash]<<"\n";
        }
